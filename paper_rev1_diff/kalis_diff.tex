...
%DIF DEL ../paper/kalis.tex        Wed Aug  9 10:22:07 2023
%DIF ADD ../paper_rev1/kalis.tex   Tue Dec 12 20:55:58 2023
...
\DIFdelbegin %DIFDELCMD < \abstract{\textbf{Background}
%DIFDELCMD < Approximating the recent phylogeny of $N$ phased haplotypes at a set of variants along the genome is a core problem in modern population genomics and central to performing genome-wide screens for association, selection, introgression, and other signals.
%DIFDELCMD < The Li \& Stephens (LS) model provides a simple yet powerful hidden Markov model for inferring the recent ancestry at a given variant, represented as an $N \times N$ distance matrix based on posterior decodings.
%DIFDELCMD < However, existing posterior decoding implementations for the LS model cannot scale to modern datasets with tens or hundreds of thousands of genomes.
%DIFDELCMD < 

%DIFDELCMD < \textbf{Results}
%DIFDELCMD < We provide a high-performance engine to compute the LS model, enabling users to rapidly develop a range of variant-specific ancestral inference pipelines on top, exposed via an easy to use package, \pkg{kalis}, in the statistical programming language \proglang{R}.
%DIFDELCMD < \pkg{kalis} exploits both multi-core parallelism and modern CPU vector instruction sets to enable scaling to problem sizes that would previously have been prohibitively slow to work with.
%DIFDELCMD < 

%DIFDELCMD < \textbf{Conclusions}
%DIFDELCMD < The resulting distance matrices accessible via \pkg{kalis} enable local ancestry, selection, and association studies in modern large scale genomic datasets.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \abstract{\textbf{Background}
...
\DIFdelbegin \DIFdel{The LS modelprovides the basis for the genome-wide }\DIFdelend \DIFaddbegin 

\DIFadd{Accelerated implementations of the LS model, typically targeting the Viterbi path, are integral to many commonly used genomics software packages, including BEAGLE \mbox{%DIFAUXCMD
\cite{beagle}}\hskip0pt%DIFAUXCMD
, IMPUTE \mbox{%DIFAUXCMD
\cite{impute}}\hskip0pt%DIFAUXCMD
, and tsinfer \mbox{%DIFAUXCMD
\cite{tsinfer}}\hskip0pt%DIFAUXCMD
.
A pioneering }\DIFaddend ancestry inference software \DIFdelbegin \DIFdel{ChromoPainter, which summarizes the }\DIFdelend \DIFaddbegin \DIFadd{package, ChromoPainter, popularized the idea of using the LS model to summarize the }\DIFaddend ancestry of \(N\) haplotypes with an \(N \times N\) similarity matrix \cite{lawson2012inference}.
...
\DIFdelbegin \DIFdel{Beyond chromosome-wide summaries, the LS model can also be used for variant-specific ancestry inference .
The contemporary importance of this is highlighted by the recent advances achieved in the RELATE software suite \mbox{%DIFAUXCMD
\cite{speidel}}\hskip0pt%DIFAUXCMD
, which uses the LS model internally to }\DIFdelend \DIFaddbegin \DIFadd{The Relate \mbox{%DIFAUXCMD
\cite{speidel} }\hskip0pt%DIFAUXCMD
software suite extended this idea to performing local (locus-specific) ancestry inference along the genome.
...
These posterior decodings are transformed to \(N \times N\) local genetic distance matrices and used to }\DIFaddend initialise variant-specific ancestral trees for downstream population genetic analyses ranging from demography to selection inference.
\DIFaddbegin 

\DIFadd{The current Relate software suite does not provide an interface for outputting the posterior decodings at a locus of interest and does not support the original LS model, only the derived allele haplotype copying model, which requires derived allele information.
%DIF > The modern ancestral recombination graph (ARG) inference software tsinfer has similar aims to Relate.
%DIF > It utilizes the Viterbi path under the LS model and a tree sequence data structure to rapidly infer marginal trees genome-wide.
%DIF > Once computed for a set of haplotypes, a tree sequence can be used to perform forward and backward recursions under the LS model that scale \(\mathcal{O}\left( \log(N) \right)\) rather than \(\mathcal{O}\left(N\right)\).
%DIF > Of course, this requires the prior computation of a tree sequence, which is itself a computationally demanding preprocessing step.
}

\DIFadd{Additionally, a LS-like model is implemented in \mbox{%DIFAUXCMD
\cite{rosen} }\hskip0pt%DIFAUXCMD
to run forward and backward recursions to variants of interest.
However, the transition kernel used is different to the original LS model: upon a recombination event the transition kernel in \mbox{%DIFAUXCMD
\cite{rosen} }\hskip0pt%DIFAUXCMD
does not permit a haplotype to continue copying from the same donor haplotype.
}

\DIFadd{The focus of }\DIFaddend \pkg{kalis} \DIFdelbegin \DIFdel{focuses on providing }\DIFdelend \DIFaddbegin \DIFadd{is to provide }\DIFaddend a high-performance engine to \DIFdelbegin \DIFdel{compute exclusively the LS model, enabling users to rapidly develop }\DIFdelend \DIFaddbegin \DIFadd{directly obtain the posterior decoding at a set of loci of interest for a dataset with hundreds of thousands of phased haplotypes. }\pkg{kalis} \DIFadd{supports the original LS model and the derived allele haplotype copying model.
It provides a simple interface to enable rapid development of }\DIFaddend a range of future variant-specific ancestral inference pipelines on top, in the easy to use statistical programming language \proglang{R} \cite{R}.
...
\DIFaddbegin \DIFadd{In other words, the donor haplotype (hidden state) that is sampled at the first variant and after every ``recombination event'' in the copying path is drawn according to \(\Pi\).
}\DIFaddend Here and whenever possible in \pkg{kalis}, all matrices are column-oriented such that the \(i\)th column pertains to an independent HMM where \(h_i\) is treated as the observation.
...
\begin{equation}
	...
	= \Pi_{ki} \rho\DIFdelbegin \DIFdel{^\ell }\DIFdelend \DIFaddbegin \DIFadd{^{\ell-1} }\DIFaddend + \mathbf{1}\left\{k = j\right\} \left(1-\rho\DIFdelbegin \DIFdel{^\ell}\DIFdelend \DIFaddbegin \DIFadd{^{\ell-1}}\DIFaddend \right) ,
	\label{eq:transition}
\end{equation}
where \(\rho^\ell = 1-\exp\left(-N_e m^\ell\right)\) and \(\mathbf{1}\left\{\cdot\right\}\) is the indicator function.
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2213} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Intuitively, this transition kernel asserts that upon a ``recombination event,'' where a recipient haplotype $i$ may change the donor haplotype $j$ it is copying from, the new donor haplotype is resampled from the prior copying distribution $\Pi_{\cdot i}$.
\mbox{%DIFAUXCMD
\cite[Appendix B]{Li2213} }\hskip0pt%DIFAUXCMD
}\DIFaddend observe that in practice the estimation of recombination rates \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend improved when the scaled recombination rate is raised to a power, so we adopt this approach and introduce an exponent \(\gamma\).
\DIFaddbegin \DIFadd{By default, }\pkg{kalis} \DIFadd{sets \(\gamma = 1\), but this can be changed by the user.
}\DIFaddend For \(\gamma>1\) the recombination map becomes more heavily peaked, whereas \(\gamma<1\) tempers the recombination map to make it more flat and smooth.
...
The \DIFaddbegin \DIFadd{parameter set used by }\pkg{kalis} \DIFadd{can be created by calling the }\texttt{\DIFadd{Parameters()}} \DIFadd{function, which retuns a }\DIFaddend \texttt{kalisParameters} object \DIFaddbegin \DIFadd{with structure shown in Table \ref{tab:pars}.
This structure corresponds to the parameters required to specify the LS model (\mbox{%DIFAUXCMD
\Cref{eq:emission,eq:transition}}\hskip0pt%DIFAUXCMD
).
To calculate $\rho$ from a recombination map, $N_e$ and $\gamma$, we also provide a helper function, }\texttt{\DIFadd{CalcRho()}}\DIFadd{, which implements \mbox{%DIFAUXCMD
\Cref{eq:rho}}\hskip0pt%DIFAUXCMD
.
}
...
\DIFaddbegin \subsection*{\DIFadd{Unit tests}}

\DIFadd{Given the complexity of the development described above, we have implemented a comprehensive suite of unit tests to ensure correctness.
Internal to the package is a ``gold master'' implementation of the LS model, which is a pure }\proglang{R} \DIFadd{implementation that has been written for correctness and is not optimised for speed.
These pure }\proglang{R} \DIFadd{implementations are callable with an undocumented argument option to the standard }\texttt{\DIFadd{Forward()}} \DIFadd{and }\texttt{\DIFadd{Backward()}} \DIFadd{functions: if the argument }\texttt{\DIFadd{nthreads = "R"}} \DIFadd{rather than a numeric value, then the gold master implementation is used (at the cost of running significantly slower).
}

\DIFadd{Unit tests fall broadly into two categories, one verifying the correctness of loading from the different supported input formats (via R matrix, }\texttt{\DIFadd{.hap.gz}} \DIFadd{and }\texttt{\DIFadd{h5}}\DIFadd{) into the optimised binary representation of \mbox{%DIFAUXCMD
\Cref{fig:hapsmem}}\hskip0pt%DIFAUXCMD
, the other checking forward and backward computations against a ground truth computed by the gold standard }\proglang{R} \DIFadd{implementation.
The latter category of tests are the most extensive, since they cover tests of all combinations of: single threaded and multi-threaded computation; moving different numbers of variants in a single call; different problem sizes where the numbers of haplotypes is either exactly divisible by the CPU vector unit length (i.e.~256-bits for AVX2 etc), or has different remainders; original LS and derived allele haplotype copying model; scalar and vector mutation probabilities (\(\mu\)); uniform and matrix copying probabilities (\(\Pi\)); and in the case of backward recursions, all combinations of starting and ending a recursion in standard or rescaled probability space (}\texttt{\DIFadd{beta.theta}} \DIFadd{argument to }\texttt{\DIFadd{Backward()}}\DIFadd{).
}

\DIFadd{All these combinations give rise to over $162,000$ tests (note also that the exact number of tests varys by architecture due to the differing vector unit lengths).
...
We note that the tests take quite some time to run (e.g.~potentially 30-60 minutes on a laptop), precisely because the gold master }\proglang{R} \DIFadd{code is run to provide the ground truth for these tests.
}

\DIFadd{If a user wishes to confirm correctness on their particular platform, they can be run with the following commands:
}

\DIFmodbegin
\begin{DIFverbatim}[alsolanguage=DIFcode]
%DIF > remotes::install_github("louisaslett/kalis",
%DIF >                         INSTALL_opts = "--install-tests")
%DIF > testthat::test_package("kalis")
\end{DIFverbatim}
\DIFmodend
...
\DIFaddbegin \subsection*{\DIFadd{Benchmarking comparison}}

\DIFadd{We performed two benchmarking experiments to compare the implementations of the forward and backward algorithms in }\pkg{kalis} \DIFadd{to those in Relate \mbox{%DIFAUXCMD
\cite{speidel}}\hskip0pt%DIFAUXCMD
.
We based all of our benchmarks off on the same set of haplotypes, taken from the 1000 Genomes Project \mbox{%DIFAUXCMD
\cite{10002015global}}\hskip0pt%DIFAUXCMD
, as used in our real-data example which follows below.
The data include 5008 haplotypes observed at 29193 variants.
}

\pkg{kalis} \DIFadd{can perform the forward and backward recursions under either the original LS model (the default) or the derived allele haplotype copying model if }\texttt{\DIFadd{use.speidel=TRUE}} \DIFadd{is passed to the }\texttt{\DIFadd{Parameters()}} \DIFadd{function.
...
This acceleration cannot be applied to the original LS model, which }\pkg{kalis} \DIFadd{was primarily designed for, because the symmetric emission kernel requires both the forward and backward algorithms to iterate over every variant for every recipient haplotype.
Even with the derived allele copying model activated, }\pkg{kalis} \DIFadd{will still visit every variant for every recipient haplotype.
}

\DIFadd{Accordingly, we found that the forward and backward recursions were approximately $4\times$ faster using Relate rather than using }\pkg{kalis}\DIFadd{.
However, if Relate visits every locus, as would be necessary to compute the original LS model, we found that the forward and backward recursions were approximately $6\times$ faster using }\pkg{kalis} \DIFadd{rather than using Relate.
This demonstrates the benefit of the low-level optimisations made in }\pkg{kalis}\DIFadd{.
In principle }\pkg{kalis} \DIFadd{could also employ the same optimisation as Relate and visit only derived sites for every recipient haplotype.
We consider this an exciting avenue of future research.
}

\DIFadd{Full details of how this benchmarking was performed are provided in Additional file 1, Section D.
}


\DIFaddend \subsection*{Real-data example: recent selection for lactase persistence}
\label{sec:realdata}
...
\DIFdelbegin \DIFdel{Using }\DIFdelend \DIFaddbegin \DIFadd{For example, \mbox{%DIFAUXCMD
\cite{busby2017inferring} }\hskip0pt%DIFAUXCMD
used }\DIFaddend another implementation of the LS model \DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\cite{busby2017inferring} }\hskip0pt%DIFAUXCMD
estimated }\DIFdelend \DIFaddbegin \DIFadd{to compare haplotypes at the }\emph{\DIFadd{LCT}} \DIFadd{locus sampled from the West African Fula population to haplotypes collected from across Europe and Asia as part of the 1000 Genomes project \mbox{%DIFAUXCMD
\cite{10002015global}}\hskip0pt%DIFAUXCMD
.
They found that the genetic distance between Fulani haplotypes and Eurasian haplotypes was unusually small at the }\emph{\DIFadd{LCT}} \DIFadd{locus.
With some further analysis, they interpreted this as evidence }\DIFaddend that a European haplotype conferring lactase persistence became prevalent within the West African Fula population due to \DIFaddbegin \DIFadd{recent }\DIFaddend natural selection sometime over the past two thousand years.

\DIFdelbegin \DIFdel{Here we }\DIFdelend \DIFaddbegin \DIFadd{Although it is difficult to directly replicate \mbox{%DIFAUXCMD
\cite{busby2017inferring} }\hskip0pt%DIFAUXCMD
since the Fulani samples they studied are not a part of the 1000 Genomes project, we take inspiration from their analysis.
...
We }\DIFaddend run \pkg{kalis} on 5008 haplotypes from the 1000 Genomes Phase 3 release to revisit the haplotype structure around \emph{LCT}; the haplotypes are sampled from 26 sub-populations all over the world \cite{10002015global}.
...
\DIFdelbegin \DIFdel{However, }\DIFdelend \DIFaddbegin \DIFadd{We cannot confirm whether any of }\DIFaddend these three haplotypes \DIFaddbegin \DIFadd{correspond to the one identified in the Fulani by \mbox{%DIFAUXCMD
\cite{busby2017inferring}}\hskip0pt%DIFAUXCMD
.
These three haplotypes }\DIFaddend are not the only structure we see: in the upper left corner of the African (AFR) block we see some haplotypes that are only found inside Africa; and in the non-African block, a haplotype that is only found outside Africa.
We can also see some sub-structure within the clear haplotype blocks.

\DIFaddbegin \DIFadd{The code to reproduce this example is available in the }\texttt{\DIFadd{examples}} \DIFadd{directory of repository associated with this paper (}\href{https://github.com/louisaslett/kalis-bmc}{\texttt{\DIFadd{https://github.com/louisaslett/kalis-bmc}}}\DIFadd{), as a vignette in the package (if vignettes built at install time), or directly at the }\pkg{kalis} \DIFadd{package website }\href{https://kalis.louisaslett.com/articles/lct_example.html}{\texttt{\DIFadd{https://kalis.louisaslett.com/articles/lct\_example.html}}}
...
%DIF < \bibliography{kalis}% common bib file
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > %%%\bibliography{kalis}% common bib file
%DIF > %%%\iffalse
\begin{thebibliography}{10}
	...
	\DIFaddbegin \bibitem{beagle}
	\DIFadd{\bibinfo{author}{Browning, B.~L.}, \bibinfo{author}{Tian, X.}, \bibinfo{author}{Zhou, Y.} \& \bibinfo{author}{Browning, S.~R.}
	}\newblock \DIFadd{\bibinfo{title}{{Fast two-stage phasing of large-scale sequence data}}.
	}\newblock \emph{\DIFadd{\bibinfo{journal}{The American Journal of Human Genetics}}} \textbf{\DIFadd{\bibinfo{volume}{108}}}\DIFadd{, \bibinfo{pages}{1880--1890} (\bibinfo{year}{2021}).
	}

	\bibitem{impute}
	\DIFadd{\bibinfo{author}{Rubinacci, S.}, \bibinfo{author}{Delaneau, O.} \& \bibinfo{author}{Marchini, J.}
	}\newblock \DIFadd{\bibinfo{title}{Genotype imputation using the positional burrows wheeler transform}.
	}\newblock \emph{\DIFadd{\bibinfo{journal}{PLoS genetics}}} \textbf{\DIFadd{\bibinfo{volume}{16}}}\DIFadd{, \bibinfo{pages}{e1009049} (\bibinfo{year}{2020}).
	}

	\bibitem{tsinfer}
	\DIFadd{\bibinfo{author}{Kelleher, J.} }\emph{\DIFadd{et~al.}}
	\newblock \DIFadd{\bibinfo{title}{Inferring whole-genome histories in large population datasets}.
	}\newblock \emph{\DIFadd{\bibinfo{journal}{Nature genetics}}} \textbf{\DIFadd{\bibinfo{volume}{51}}}\DIFadd{, \bibinfo{pages}{1330--1338} (\bibinfo{year}{2019}).
	}

	\DIFaddend \bibitem{lawson2012inference}
	...
	\DIFaddbegin \bibitem{rosen}
	\DIFadd{\bibinfo{author}{Rosen, Y.~M.} \& \bibinfo{author}{Paten, B.~J.}
	}\newblock \DIFadd{\bibinfo{title}{{An average-case sublinear forward algorithm for the haploid Li and Stephens model}}.
	}\newblock \emph{\DIFadd{\bibinfo{journal}{Algorithms for Molecular Biology}}} \textbf{\DIFadd{\bibinfo{volume}{14}}}\DIFadd{, \bibinfo{pages}{1--12} (\bibinfo{year}{2019}).
	}
	...
	\DIFaddbegin \bibitem{10002015global}
	\DIFadd{\bibinfo{author}{Consortium, . G.~P.} }\emph{\DIFadd{et~al.}}
	\newblock \DIFadd{\bibinfo{title}{A global reference for human genetic variation}.
	}\newblock \emph{\DIFadd{\bibinfo{journal}{Nature}}} \textbf{\DIFadd{\bibinfo{volume}{526}}}\DIFadd{, \bibinfo{pages}{68} (\bibinfo{year}{2015}).
	}

	\DIFaddend \bibitem{ingram2009lactose}
	...
	\DIFdelbegin \bibitem{10002015global}
\DIFdel{\bibinfo{author}{Consortium, . G.~P.} }\emph{\DIFdel{et~al.}}
%DIFAUXCMD
%DIFDELCMD < \newblock %%%
\DIFdel{\bibinfo{title}{A global reference for human genetic variation}.
}%DIFDELCMD < \newblock %%%
\emph{\DIFdel{\bibinfo{journal}{Nature}}} %DIFAUXCMD
\textbf{\DIFdel{\bibinfo{volume}{526}}}%DIFAUXCMD
\DIFdel{, \bibinfo{pages}{68} (\bibinfo{year}{2015}).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem{sokal1958statistical}
